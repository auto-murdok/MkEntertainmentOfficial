...
- **Scripts Directory:** `Assets/_Game/Scripts/`
  ...
  - `Core/UI/`: Character UI controller/elements, `PlayerHud` (visible gameplay HUD: HP, ammo, combat ticker — now decoupled via `IHealthSource`/`IWeapon` in `Core/Interfaces/`), `DebugHud` (hidden F3 diagnostics overlay — intentional diagnostic coupling, the only `Game.UI → Game.Characters/Items` edge), `MainMenuController` (menu canvas + start/host/join/quit flow), `PauseMenuController` (Esc/Start overlay in arenas: resume / quit-to-menu with session shutdown).
  ...
  - `Core/Interfaces/`: Cross-feature contracts (`IHealthSource`, `IWeapon`, `IPlayerInputGate`, `IDamageable`, `IInteractable`, `IPlayerBiteRelay`) — the seam that lets `Characters`, `Items`, `UI` depend only on `Game.Core` (see 2026 modularity audit below).
  - `Items/`: ItemCatalog (SO asset), weapons, firearm events and gun contexts, `AmmoPickup` (zombie ammo drops / reserve refill).
  - **Assembly definitions (modularity — Unity 6 gold standard, audited 2026-08-31):** `Game.Core` (no domain deps), `Game.Items` → `Game.Core`, `Game.Characters` → `Game.Core` + `Game.Items` (only for weapon equip — next iteration will inject `IWeapon` from composition root to drop this edge), `Game.UI` → `Game.Core` (+ `Characters/Items` only for `DebugHud`; `PlayerHud` is already Core-only via `IHealthSource`/`IWeapon`), `Game.Composition` → all (the sole aggregator), `Game.Editor` → `Core/Characters/Items`. All use `autoReferenced:false` + GUID refs (`Use GUIDs` best practice) so the graph is explicit; `CharacterBrain: IHealthSource` and `Weapon: IWeapon` provide the UI seam, `CharacterLocomotion → ItemCatalog` is the remaining explicit debt.
- **ScriptableObject architecture (modularity):** entities never reach out to scene objects or static singletons.
  ...
  - **Arena spawning (test maps):** `ZombieSpawner` (`Characters/AI/Entities/ZombieAI/Components/ZombieSpawner.cs:10`) holds `Spawn_*/Spawn_Atrium_*` children. `Start()` calls `SpawnInitialWave()` (one zombie per `SpawnPoints` entry, culled by `_maxZombies`). Networked arena `MercyHospital/Floor_Atrium` spawns (center/NE/NW/South/North) guarantee visible horde on the atrium even inside the building; `ExpandedCombatArena` mirrors with center spawns. All arena `Spawn_` children are now assigned to `_spawnPoints` (13 in `NetworkedCombatArena`, 12 in `ExpandedCombatArena`) so no manual placement is missed. Interval spawns continue via `_spawnInterval` (30 s).
  - **Networking (milestone 3):** `NetworkedCombatArena` is the NGO playground (`NetworkManager` + `UnityTransport`; roles via the menu **HOST/JOIN GAME** buttons (localhost 127.0.0.1:7777, `NetworkSession.desiredMode`) or the command line — no args hosts, `-mlclient`/`-client` joins). `NetworkManager.PlayerPrefab` = `FemaleCharacter` (carries `NetworkObject` + **owner-authoritative** `NetworkTransform` (`AuthorityMode=Owner` — required for client movement; keep server authority for future server-simulated NPCs) + **owner-authoritative** `NetworkAnimator` (params/states sync automatically; triggers MUST route through `CharacterLocomotion.SetAnimatorTrigger` — `Animator.SetTrigger` never replicates; remote copies get `applyRootMotion=false`, the FSM disabled, and a rebuilt rig graph pointing at a local `RemoteAimTarget` — aim visuals replicate via `NetworkVariable<bool> _isAiming`) + `NetworkedPlayerComposition` — the owner composes the local rig in `OnNetworkSpawn`; remote players must early-out of input wiring). **Zombies are server-simulated** (`ZombieSpawner` spawns on the host with `NetworkObject.Spawn`; `NetworkTransform`/`NetworkAnimator` stay server-authoritative — zombie triggers route via `ZombieContext.SetAnimatorTrigger`; `NetworkedZombieController` disables the FSM on clients and despawns on death while the host keeps the ragdoll). **Health is server-authoritative** (`NetworkedHealth` server-write `NetworkVariable` + `ActorBrainBase.MirrorHitPoints` on peers) and **so is bullet damage** (`BulletProjectile` → `NetworkedDamage.Apply` → `NetworkedDamageRelay.ReportDamageServerRpc` on the victim; the shooter never applies HP locally in a session). **Bites replicate via the owner relay** (`IPlayerBiteRelay` → targeted ClientRpc to the victim's owner, whose FSM runs take-bite and replicates through its NetworkAnimator; the owner mirrors bite state back via owner-write NetworkVariables so `IBiteTarget` checks stay correct on the host; zombie `isPreparing` is server-replicated for the victim's pin phase). **Death replication**: mirrored deaths on FSM-less remote copies must call `ActorBrainBase.RunDeathTeardown()` (the Dead state never fires there); `GameStateManager.freezeTimeOnGameOver` is false in networked scenes (freezing one peer's time streams a frozen standing ragdoll to everyone); `RagdollUtils.EnableRagdoll` wakes + topples the standing skeleton (a balanced pose is a stable physics tower). **Zombie corpses** linger for `ZombieData.corpseDestroyDelay` (10 s) on every peer (no instant despawn) and dead actors move off the `LocalPlayer` layer in `OnRagdollEnabled` (the AI vision scan is layer-based — corpses must not be scannable). **Overlays gate input** (`IPlayerInputGate`/`PlayerInputGate` on the composition root, injected into the pause menu + GameStateManager): pause/game-over disable the `PlayerInput` and zero held values; `CharacterBrain` re-evaluates held sprint/aim from the live action state every frame (event-latched held states stick if a release is ever missed). Ragdoll teardown must **disable** (never destroy) the `NetworkAnimator` and `Animator` — destroying NetworkBehaviours on one peer shifts NGO's RPC routing indices ("NetworkBehaviour index out of bounds" NREs), and a disabled-but-alive Animator is safe for the NetworkAnimator's poll. Client builds: `unity command build` → run `Builds/NetworkClient.exe -mlclient`. The old `ZombieCombatArena` scene was removed. **Full command cheat-sheet + gotchas: `docs/networking_notes.md`; full CLI harness: `docs/cli_testing.md` (`--scene/--mode/--connect/--autoStart/--autoQuit/--noSpawning/--godMode/--infiniteAmmo/--seed/--timeScale`).**
...
- **Shooting engine:**
  ...
  - **CLI test harness (automated agents):** `Core/Cli/GameCliArgs.cs:1` (static parser, case-insensitive `-key value` / `--key=value`, `SetArgsForTesting/ResetForTesting`) + `Composition/GameCliBootstrap.cs:1` (`RuntimeInitializeOnLoadType.BeforeSceneLoad` DontDestroyOnLoad — `--scene`, `--mode host|client|auto` + legacy `-mlclient`/`-client`, `--connect`/`--address`/`--port` → `NetworkSession.EffectiveAddress/EffectivePort`, `--autoStart` skips `MainMenu`, `--autoQuit`/`--quitAfter`/`--maxDuration`/`--duration` realtime quit, `--noSpawning`/`--maxZombies`/`--spawnInterval`/`--timeScale`/`--seed`, `--godMode` player-only in `ActorBrainBase:46`, `--infiniteAmmo` in `Weapon:22`/`HandgunReloadingState:29`, `--help`/`--verbose`/`--automated`). Gold standard: Unity Manual *Command line arguments* + batchmode + NGO quickstart. Full reference + recipes: `docs/cli_testing.md`.

---

## 🧪 Testing (Unity Test Framework — keep the suite green)

The project has a regression suite in `Assets/_Game/Tests/` (EditMode + PlayMode assemblies, 238 tests: 108 EditMode + 130 PlayMode). **Run it after every behaviour change** and **add/extend tests for any new gameplay logic** — see `docs/testing.md` for the full guide, coverage map and gotchas. CLI parsing is covered by `GameCliArgsTests` in `Game.Tests.EditMode` — see `docs/cli_testing.md` for the full CLI harness.
...
## 🤖 CLI Automation — Automated Agent Test Harness

The game is **entirely testable via CLI args** (Editor and built Player) so an automated agent can drive any flow without UI clicks. Single source of truth: `Core/Cli/GameCliArgs.cs:1` (static, case-insensitive, `-key value` / `--key=value` / flag, `HasFlag`/`GetValue`/`GetInt`/`GetFloat`, `SetArgsForTesting`).

Bootstrap: `Composition/GameCliBootstrap.cs:1` (`RuntimeInitializeOnLoadType.BeforeSceneLoad`, hidden `DontDestroyOnLoad` GO):
- `--help` logs `HelpText` and quits in batchmode, `--verbose` dumps `Dump()`
- pushes `--connect`/`--address`/`--port` into `NetworkSession.OverrideAddress/OverridePort` → `EffectiveAddress/EffectivePort` consumed by `NetworkArenaBootstrap:44` before `StartHost/StartClient` (menu `desiredMode` wins when explicitly set, otherwise `--mode host|client|auto` + legacy `-mlclient`/`-client`)
- redirects `--scene <name>` when active scene differs (Build Settings check), `--autoStart` skips `MainMenu` via `MainMenuController` (host/join/start)
- schedules `--autoQuit`/`--quitAfter`/`--maxDuration`/`--duration` via `WaitForSecondsRealtime` + `Application.Quit`/`EditorApplication.Exit`
- applies `--noSpawning`/`--maxZombies`/`--spawnInterval`/`--timeScale`/`--seed` after `sceneLoaded`, `--godMode` (player-only in `ActorBrainBase:46`) and `--infiniteAmmo` (`Weapon:22`/`HandgunReloadingState:29`) read directly at gameplay time

Agent recipes (see `docs/cli_testing.md` for the full reference):

```bash
# Smoke: no spawns, deterministic, auto-exit, headless log
Unity.exe -batchmode -projectPath . --scene ExpandedCombatArena --noSpawning --seed 123 --autoQuit 10 -quit -logFile smoke.log
# Host + client on one box
...
# Editor fast iteration (skip menu, god, infinite ammo)
Unity.exe -projectPath . --scene NetworkedCombatArena --mode host --autoStart --noSpawning --infiniteAmmo --godMode
```

Tests: `GameCliArgsTests` (EditMode) covers parser, mode/connect aliases, auto-quit, gameplay overrides, help, `IsCommandLineClient` delegation, and `NetworkSession` effective address.
