...
public class MainMenuController : MonoBehaviour
{
    ...
    private static readonly Color Accent = new Color(0.72f, 0.09f, 0.07f, 1f);
    private static readonly Color AccentBright = new Color(0.95f, 0.22f, 0.13f, 1f);
    private static readonly Color ButtonIdle = new Color(0.92f, 0.88f, 0.86f, 0.82f);
    private static readonly Color ButtonHover = new Color(1f, 0.97f, 0.94f, 1f);
    private static readonly Color PanelTint = new Color(0.10f, 0.02f, 0.02f, 0.6f);
    ...
    public void BuildUI()
    {
        ...
        CreateImage(canvasGo.transform, "Backdrop", VerticalGradientTexture(BackdropTop, BackdropBottom), Color.white);
        CreateImage(canvasGo.transform, "Vignette", VignetteTexture(), new Color(0f, 0f, 0f, 0.85f));

        BuildTitle(canvasGo.transform);
        BuildButtons(canvasGo.transform);

        _fadeOverlay = CreateImage(canvasGo.transform, "FadeOverlay", Texture2D.whiteTexture, new Color(0f, 0f, 0f, 0f));
    ...
    {
        CreateMenuButton(parent, "StartGameButton", "START GAME", new Vector2(0.5f, 0.42f), StartGame);
        CreateMenuButton(parent, "QuitButton", "QUIT", new Vector2(0.5f, 0.27f), QuitGame);
    ...
    private Button CreateMenuButton(Transform parent, string name, string label, Vector2 anchor, Action onClick)
    {
        ...
        panelImage.sprite = PanelTexture();
    ...
    private static Image CreateImage(Transform parent, string name, Texture2D spriteSource, Color color)
    {
        GameObject go = new GameObject(name);
        go.transform.SetParent(parent, false);
        Image image = go.AddComponent<Image>();
        image.sprite = Sprite.Create(spriteSource, new Rect(0f, 0f, spriteSource.width, spriteSource.height), new Vector2(0.5f, 0.5f));
        image.color = color;
    ...
    private static void ApplyVertexGradient(TMP_Text text)
    {
        ...
        text.colorGradient = new VertexGradient(Color.white, new Color(1f, 0.62f, 0.55f), Accent, new Color(0.4f, 0.03f, 0.03f));
    }

    private static Texture2D VerticalGradientTexture(Color top, Color bottom)
    {
    ...
    private static Sprite PanelTexture()
    {
        const int size = 48;
        var texture = new Texture2D(size, size, TextureFormat.RGBA32, false);
        for (int y = 0; y < size; y++)
    ...
    private static void EnsureEventSystem()
    {
        ...
        // StandaloneInputModule breaks under "Input System only".
